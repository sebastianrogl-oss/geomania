...
class FlagQuizScreen extends StatefulWidget {
  ...
  const FlagQuizScreen({super.key, this.questionCount = 10, this.isDaily = false, this.continent});
...
class _FlagQuizScreenState extends State<FlagQuizScreen> {
  ...
  void _startQuiz() {
    ...
    _options = _questions.map((q) {
      final wrongs = (List<Country>.from(countries)
            ..removeWhere((c) => c.name == q.name)
            ..shuffle(rng))
          .take(_optionsCount - 1)
          .map((c) => c.name)
          .toList();
      return ([q.name, ...wrongs]..shuffle(rng));
    }).toList();
  ...
  void _answer(String choice) {
    ...
    final correct = choice == _questions[_current].name;
    ...
    Future.delayed(const Duration(milliseconds: 900), () {
      ...
      if (_current + 1 >= _totalQ) {
        StatsService.saveResult(category: 'flags', isDaily: widget.isDaily, score: _score, total: _totalQ);
        if (!widget.isDaily) {
          StatsService.saveCountryAnswers(category: 'flags', answers: Map.of(_countryAnswers));
          StatsService.saveLernenDoneToday('flags');
        }
        ...
        child: Column(
          children: [
            ...
            ...opts.map((opt) => _OptionButton(
                  label: opt,
                  correct: _questions[_current].name,
                  selected: _selected,
                  onTap: () => _answer(opt),
